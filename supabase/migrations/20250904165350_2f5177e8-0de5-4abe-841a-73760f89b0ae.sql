...
ON public.messages 
FOR DELETE 
USING (auth.uid()::text = (SELECT id::text FROM public.profiles WHERE username = messages.username));
...
-- Update existing messages to have user_id (this is a one-time migration)
-- Since we don't have auth.users data for existing messages, we'll need to handle this in the app
...
-- First, let's make sure the bucket exists and is NOT public
